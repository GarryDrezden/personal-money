...
if ($uri === '/health' && $method === 'GET') {
    ...
    jsonResponse([
        'ok' => true,
        'php' => PHP_VERSION,
        'configExists' => is_readable($configPath),
        'dbDriver' => $config['DB_DRIVER'] ?? 'sqlite (default)',
...
try {
    ...
    $hint = is_readable(__DIR__ . '/config.php')
        ? 'Check MySQL credentials in api/config.php'
        : 'Create api/config.php on the server (see config.example.php)';
    jsonError($hint . ': ' . $e->getMessage(), 500);
}
